namespace EShop.Core.Constants
{
    public static class Messages
    {
        public const string UserRegisteredSuccessfully = "ثبت‌نام کاربر با موفقیت انجام شد.";
        public const string LoginFailed = "نام کاربری یا رمز عبور اشتباه است.";
        public const string ProductAddedSuccessfully = "محصول با موفقیت اضافه شد.";
        public const string ProductNotFound = "محصول موردنظر یافت نشد.";
        public const string OrderPlaced = "سفارش شما با موفقیت ثبت شد.";
        public const string UnauthorizedAccess = "شما اجازه دسترسی به این بخش را ندارید.";
    }
}
